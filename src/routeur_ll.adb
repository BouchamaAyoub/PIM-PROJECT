   ...
   when DATA_ERROR =>
    ...
    -- Exécuter la commande.
     if Commande = "TABLE" then
      ...
      Afficher_Cache_LL(Cache, Politique); -- Afficher le cache.
    else
      Put_Line("Commande inconnue à la ligne " & Integer'Image(Numero_ligne)); -- Afficher la ligne de la commande inconnue.
    end if;
   when E : others =>
