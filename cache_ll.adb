...
package body Cache_LL is
    ...
    begin
        Put_Line("Nombre d'appels : " & Integer'Image(Cache.N_Appels));
        Put_Line("Nombre de défauts : " & Integer'Image(Cache.N_Defauts));
        if Cache.N_Appels = 0 then
            Put_Line("Taux de défauts incalculable.");
        else
            Put_Line("Taux de défauts : " & Float'Image(Float(Cache.N_Defauts) / Float(Cache.N_Appels)));
        end if;
    ...
    procedure Afficher_Cache_LL (Cache : in T_Cache; Politique : in Unbounded_String) is
        ...
        begin
            ...
            if Politique = To_Unbounded_String("LFU") then
                Put(" - " & Integer'Image(Donnee.N_Appels_IP));
            end if;
            New_Line;
        end Afficher_Liste;
        ...
        if Taille(Cache.L) > Cache.Size_Max then
            if Politique = To_Unbounded_String("LRU") or Politique = To_Unbounded_String("FIFO") then
                ...
                for i in 1..(Taille(Cache.L)-1) loop
                    ...
                    if Min_Cellule.N_Appels_IP < Min then
                        Min := Min_Cellule.N_Appels_IP;
                        Min_Dest := Dest;
                    end if;
                end loop;
    ...
    procedure Incrementer_Defauts_LL (Cache : in out T_Cache) is
    begin
        Cache.N_Defauts := Cache.N_Defauts + 1;
    end Incrementer_Defauts_LL;
    ...
    function Nb_Appels_Destination_LL (Cache : in T_Cache; Destination : in T_Adresse_IP) return Integer is
        Donnee : T_Cellule;
    begin
        if Cle_Presente(Cache.L, Destination) then
            Donnee := La_Donnee(Cache.L, Destination);
            return Donnee.N_Appels_IP;
        else
            return 0;
        end if;
    end Nb_Appels_Destination_LL;
